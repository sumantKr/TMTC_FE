...
const EventGroup = ({
    ...
    return (
        <div className="h-20 border-t last:border-b">
            {events
                .filter((event) => isSameHour(new Date(event.startDate), hour))
                .map((event) => {
                    const hoursDifference =
                        differenceInMinutes(new Date(event.endDate), new Date(event.startDate)) / 60;
                    const startPosition = new Date(event.startDate).getMinutes() / 60;
                    ...
                    return (
                        ...
                        >
                            {event.title} - {event.destination} - ${event?.budget}
                        </div>
                    );
                })}
        </div>
            ...
            <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
                {monthDates.map((_date) => {
                    const currentEvents = events.filter((event) =>
                        isSameDay(new Date(event.startDate), _date)
                    );
